...
function Resource(shippo, urlData) {
  ...
  if (this._shippo._api.merchantId) {
    this.path = `/merchants/${this._shippo._api.merchantId}/${this.path}`;
  }
...
Resource.prototype = {
    ...
    return function (res) {
      ...
      res.on('end', function () {
        ...
        try {
          ...
          return callback.call(self, new Error.ShippoAPIError(errData), null);
        }
  ...
  _request: function (method, path, data, auth, callback) {
    ...
    function makeRequest() {
      ...
      var req = (self._shippo.get('protocol') == 'http' ? http : https).request(request_obj);
