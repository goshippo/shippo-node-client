        ...
        try {
          ...
          return callback.call(
            ...
            new Error.ShippoAPIError({
              ...
              developerMessage: response.detail,
              response: response
            }), null);
        }
        if (res.statusCode === 401) {
          err = new Error.ShippoAuthenticationError({ 
            message: "Invalid credentials",
            developerMessage: response.detail,
            response: response
          });
        } else if (res.statusCode === 404) {
          err = new Error.ShippoNotFoundError({
          ...
          err = new Error.ShippoAPIError({
            message: 'API sent us a 301 redirect, stopping call. Please contact our tech team (support@goshippo.com) and provide them with the operation that caused this error.',
            developerMessage: response.detail,
            response: response
          });
